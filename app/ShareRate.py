...
def shares_parser(task_list):
    """
    Parses the stock prices from Google search results for a list of stock symbols.

    Args:
        task_list (list): List of stock symbols to search for.

    Returns:
        dict: A dictionary with stock symbols as keys and a tuple with the price in string format
              and price as a float value.
    """
    result = {}
    for i in task_list:
        ...
        params = {
            'q': f'стоимость акции {i}',  # Query string: stock price for symbol `i`
        ...
        result[f'{i}'] = ('$' + final_numb, float(final_numb.replace(',', '.')))

    return result

def main():
    # Example stock symbols
    res = shares_parser(['googl', 'aapl'])
